...
export function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-10 bg-surface/95 backdrop-blur border-t border-rim">
        ...
        {tabs.map(({ href, label, Icon }) => {
          ...
          return (
            <Link
              key={href}
              href={href}
              className={`flex-1 flex flex-col items-center justify-center py-2 gap-1 min-h-[56px] transition-colors duration-150 ${
                active ? "text-accent" : "text-faint"
              }`}
            >
              <Icon active={active} />
              <span className="text-[10px] font-medium uppercase tracking-[0.07em]">{label}</span>
              <span className={`w-1 h-1 rounded-full transition-all duration-150 ${active ? "bg-accent" : "bg-transparent"}`} />
            </Link>
          );
        })}
      </div>
