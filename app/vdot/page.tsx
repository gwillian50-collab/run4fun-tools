    ...
    <div className="max-w-2xl mx-auto px-4 pt-5 flex flex-col gap-4 pb-24">
      ...
      <ExpandableSection title="O que é o VDOT?" detailsRef={ref}>
        <div className="flex flex-col gap-1">
          <span className="text-[11px] font-medium text-content uppercase tracking-[0.07em]">O conceito</span>
        ...
        <div className="flex flex-col gap-1">
          <span className="text-[11px] font-medium text-content uppercase tracking-[0.07em]">Como é calculado</span>
        ...
        <div className="flex flex-col gap-1">
          <span className="text-[11px] font-medium text-content uppercase tracking-[0.07em]">Sobre as equivalências</span>
          ...
          <p className="text-faint mt-1">Esses números são norteadores, não limites. Resultados reais dependem de especificidade do treino, clima e estratégia de prova.</p>
        </div>

        <div className="flex flex-col gap-1">
          <span className="text-[11px] font-medium text-content uppercase tracking-[0.07em]">Os ritmos de treino</span>
          <p>O VDOT define cinco zonas, cada uma com um propósito fisiológico distinto:</p>
          <div className="flex flex-col gap-2 mt-1">
            {[
              { color: "text-success", label: "Easy (E)", desc: "A maior parte do volume semanal. Ritmo confortável onde é possível conversar. Desenvolve a base aeróbica, capilares musculares e eficiência no uso de gordura como combustível." },
              ...
              { color: "text-warning", label: "Threshold (T)", desc: "Limiar lático — o ritmo mais alto que pode ser mantido por cerca de 1 hora. Eleva o teto de velocidade sustentável. Usado em tempo runs de 20–30 min ou cruise intervals." },
              { color: "text-content", label: "Interval (I)", desc: "Maximiza o VO₂max. Esforço intenso de 3–5 min com recuperação ativa igual ao tempo do esforço. Melhora a capacidade aeróbica máxima." },
              { color: "text-danger", label: "Repetition (R)", desc: "Mais rápido que Interval, com recuperação completa. Foco em economia de corrida e velocidade neuromuscular. Repetições curtas de 200–400m." },
            ].map((z) => (
              <div key={z.label} className="flex gap-3">
                <span className={`text-[11px] font-medium ${z.color} w-24 shrink-0 pt-0.5`}>{z.label}</span>
                <p className="text-[11px] text-faint leading-relaxed">{z.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-1">
          <span className="text-[11px] font-medium text-content uppercase tracking-[0.07em]">Como isso te ajuda a melhorar</span>
